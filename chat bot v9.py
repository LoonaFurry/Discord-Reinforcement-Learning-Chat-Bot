...
# Save chat history to the database (Modified)
...
# Get relevant chat history for user (Modified)
    ...
    async with aiosqlite.connect(DB_FILE) as db:
        async with db.execute(
            ...
            async for row in cursor:
                message_text = row[0]
                bot_name = row[1]
                # Add bot name to message for context
...
async def perform_advanced_reasoning(query, relevant_history, summarized_search, user_id):
    ...
    if user_id and user_profiles[user_id]["context"]:
        context_str = "Önceki konuşma özeti:\n"
        for turn in user_profiles[user_id]["context"]:
            context_str += f"Kullanıcı: {turn['query']}\n"


    # Prompt Engineering
    personality = user_profiles[user_id]["personality"] if user_profiles[user_id]["personality"] else "neutral"
    prompt = f"Sen Türkçe konuşan, dost canlısı bir Furry genç Protogen'sin. {context_str} Kullanıcı {query} dedi. Şimdi ona uygun bir şekilde cevap ver."  # Simplified prompt, only Turkish instruction
