...
# --- Topic Model ---
class TopicModel:
    def __init__(self, model_type="lda", num_topics=5):
        ...
        if self.model_type == "lda":
            ...
            self.tokenizer = AutoTokenizer.from_pretrained("google-t5/t5-3b")  # Assuming you have access to this model
            self.model = AutoModelForSequenceClassification.from_pretrained("google-t5/t5-3b")  # Assuming you have access to this model
        else:
    ...
    def predict_topic(self, query):
        if self.model_type == "lda":
            ...
            topic_vector = outputs.logits.softmax(dim=-1).detach().numpy()[0]  # 0.index
        else:
            raise ValueError("Geçersiz konu modeli türü.")
...
class QuestionAnalyzer:
    def __init__(self):
        self.nlp = load("xx_sent_ud_sm")  # Use a larger SpaCy model
...
async def on_message(message):
    ...
    try:
        ...
        if bot.user.mentioned_in(message) or bot.user.name in message.content:
            ...
            # --- Konu Tahmini ---
            topic_model = TopicModel(model_type="bert")
            topic_vector = topic_model.predict_topic(content)
            ...
            user_profiles[user_id]["context_stack"].push(context)

            # --- Soru Analizi ---
            question_analyzer = QuestionAnalyzer()
            question_type, entities = question_analyzer.analyze(content)
