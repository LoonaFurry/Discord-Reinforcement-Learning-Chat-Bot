...
async def complex_dialogue_manager(user_profiles, user_id, message):
    if user_profiles[user_id]["dialogue_state"] == "planning":
        ...
        if user_profiles[user_id]["planning_state"]["stage"] == "initial_request":
            ...
            else: user_profiles[user_id]["planning_state"]["stage"] = "gathering_information"
...
async def perform_very_advanced_reasoning(query, relevant_history, summarized_search, user_id, message, content):
    ...
    # Update user personality based on sentiment
    if user_id:
        if sentiment > 0.5:
    ...
    try:
        ...
        conversation_history = [{"state": turn["state"], "query": turn["query"]} for turn in user_profiles[user_id]["context"]]
        ...
        if user_id:
            ...
            # Handle None state from dialogue tracker
            if next_state is None:
                logging.warning("Dialogue state tracker returned None. Using default transition.")
                next_state = dialogue_state_tracker.states[current_state].get("default_transition", "general_conversation")
            ...
            if "entry_action" in dialogue_state_tracker.states[next_state]:
                ...
                entry_action_response = entry_action(user_id)
        ...
        prompt = (
            f"You are a friendly and helpful AI assistant who understands human emotions and social cues.  "
