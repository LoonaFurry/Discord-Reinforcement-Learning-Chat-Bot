...
class DialogueStateTracker:
    states = {
        ...
        'error': {'entry_action': 'handle_error'}  # Add an error state
    }
    ...
    def __init__(self):
        ...
        self.machine.add_transition('error', '*', 'error')  # Add a transition to the error state
    ...
    def handle_error(self, user_id):  # Function for the error state
    ...
    async def classify_dialogue_act(self, user_input):
        # Use Gemini to classify dialogue act, handling errors
        for attempt in range(100):  # Retry up to 100 times
            try:
                prompt = (
...
@backoff.on_exception(backoff.expo, requests.exceptions.RequestException, max_tries=100)  # Increased retries to 100
async def generate_response_with_gemini(prompt, user_id=None):
    ...
    # Removed redundant retry loop as backoff decorator handles it
...
async def fix_link_format(text):
    """
    Fixes the link format in the text, removes special characters from URLs,
    prevents duplicate links, and ensures URLs are in the desired format (plain URLs).
    Handles multiple link formats (Markdown, plain URLs, URLs with extra symbols).
    """

    # 1. Find all potential URLs in the text (including those in Markdown format)
    ...
    for url in urls:
        # 2. Clean each URL 
        cleaned_url = await clean_url(url)
        if cleaned_url and cleaned_url not in seen_urls:
            cleaned_urls.append(cleaned_url)
            seen_urls.add(cleaned_url)

    # 3. Join the cleaned URLs with newlines
    return "\n".join(cleaned_urls) 


async def clean_url(url, description=None):
    """
    Cleans a URL by removing special characters, Markdown symbols 
    (parentheses and brackets), and potentially extracting a 
    better URL from the description if the original URL is invalid.
    """
    cleaned_url = re.sub(r"[^a-zA-Z0-9./?:]", "", url)  # Remove special characters
    cleaned_url = cleaned_url.replace("(", "").replace(")", "").replace("[", "").replace("]", "") # Remove Markdown symbols

    try:
        async with aiohttp.ClientSession() as session:
            async with session.head(cleaned_url) as response:
                if response.status != 200: # If the URL is invalid
                    if description:
                        better_url = await extract_url_from_description(description)
                        if better_url:
                            cleaned_url = better_url
                    else: 
                        cleaned_url = None # If no description, set URL to None
    except Exception as e:
        logging.error(f"Error validating URL: {e}")
        cleaned_url = None # Set URL to None in case of error
...
async def perform_very_advanced_reasoning(query, relevant_history, summarized_search, user_id, message, content):
    ...
    try:
        ...
        async def on_error(user_id):
            logging.info("Executing 'on_error' state transition function.")
            user_profiles[user_id]["last_bot_action"] = "handled_error"
            return "I'm having a little trouble understanding. Could you please rephrase your request?"
        ...
        state_transition_functions = {
            ...
            "error": on_error  # Add the error state function
        }
        ...
        try:
            ...
            response_text = response_text.replace("Gemini: ", "") # Remove "Gemini: " 
        except Exception as e:
...
async def on_message(message):
    try:
        ...
        # Send the response, handling Discord message length limits
        max_message_length = 2000
        if len(response) > max_message_length:
            response_chunks = [response[i:i+max_message_length] for i in range(0, len(response), max_message_length)]
        ...
        # Don't send error message to Discord chat, it's already handled in state transition
