...
# Define the Bayesian network structure directly during initializationv
# Get all CPDs
        ...
        if not db_exists:
            ...
            await db.execute('''
                CREATE TABLE chat_history (
                    ...
                    bot_id TEXT,
                    bot_name TEXT, 
                    selected_action INTEGER  -- Add a column to store the selected action
                )
            ''')
            await db.execute('''
            ...
            # Check if selected_action column exists in chat_history
...
async def process_db_queue():
    while True:
        ...
        try:
            async with aiosqlite.connect(DB_FILE) as db:
                await db.execute(
                    'INSERT INTO chat_history (user_id, message, timestamp, user_name, bot_id, bot_name, selected_action) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
async def on_ready():
    ...
    bot.loop.create_task(retrain_rl_agent())  # Schedule periodic retraining
