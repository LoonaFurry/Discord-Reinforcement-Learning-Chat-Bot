...
async def clean_url(url: str, description: str = None) -> str:
    ...
    # 3. Handle specific website patterns (optional but recommended)
    # Example: Ensure correct YouTube URL format
    if "youtube.com" in cleaned_url and not cleaned_url.startswith("https://www.youtube.com"):
        cleaned_url = "https://www.youtube.com/" 

    # 4. Remove extraneous characters (keep query parameters)
...
async def on_message(message: discord.Message):
    try:
        ...
        # Apply link fixes:
        response_text = remove_duplicate_links(response_text)
        response_text = fix_link_format(response_text)

        # Send the response, handling Discord message length limits
        max_message_length = 2000
        if len(response_text) > max_message_length:
            response_chunks = [response_text[i:i+max_message_length] for i in range(0, len(response_text), max_message_length)]
