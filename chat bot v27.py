...
summarizer = pipeline("summarization", model="facebook/bart-large-cnn")  # Example summarizer
...
class DialogueStateTracker:
    states = ['greeting', 'general_conversation', 'storytelling', 'question_answering', 'planning', 'farewell']

    def __init__(self):
        self.machine = Machine(model=self, states=DialogueStateTracker.states, initial='greeting')
...
async def gemini_search_and_summarize(query) -> str:
    try:
        ddg = AsyncDDGS()
        search_results = await asyncio.to_thread(ddg.text, query, max_results=100)  # Reduce max_results
        ...
        max_summary_length = min(8192, len(prompt) // 2)  # Half the prompt length, max 8192 (Gemini's limit) 
...
async def complex_dialogue_manager(user_profiles, user_id, message):
    ...
    if user_profiles[user_id]["dialogue_state"] == "planning":
        ...
        elif user_profiles[user_id]["planning_state"]["stage"] == "executing_plan": # Fixed line
            execution_result = await monitor_plan_execution(
                user_profiles[user_id]["planning_state"]["plan"], user_id, message  # Fixed line
            )
            return execution_result
    ...
    if await has_enough_planning_information(user_id):
        ...
        goal = user_profiles[user_id]["planning_state"].get("goal")
        preferences = user_profiles[user_id]["planning_state"].get("preferences")
        
        # Safety check for missing data
        if not goal or not preferences:
            return "Goal or preferences are missing. Please provide more information."

        plan = await generate_plan(goal, preferences, user_id, message)
        is_valid, validation_result = await validate_plan(plan, user_id)
...
async def perform_very_advanced_reasoning(query, relevant_history, summarized_search, user_id, message, content):
    ...
    if user_id and user_id in user_profiles:
        ...
        sentiment = 0  # You might want to set a default sentiment value
    ...
    # Check for unexpected responses from Gemini (outside the try...except block)
    ...
    if user_id and user_id in user_profiles:  # Check if user profile exists
        ...
        try:
            ...
            next_state = await dialogue_state_tracker.transition_state(current_state, query, user_id,
                                                                      user_profiles[user_id]["context"])  # Pass context

        if next_state is None:
            logging.warning("Dialogue state tracker returned None. Using default transition.")
            next_state = "general_conversation"  # Default fallback

        user_profiles[user_id]["dialogue_state"] = next_state
        # ... (Handle state-specific entry actions if needed)

    # --- Retrieve Relevant Past Data from Knowledge Graph ---
    if user_id:  # Only retrieve if user_id is available
    ...
    # --- Improving Contextual Understanding: Use Transformer-Based Summarization ---
    # FIX: Ensure max_history_summary_length is at least min_length (50)
    max_history_summary_length = max(50, len(relevant_history) // 2) 
    summarized_history = summarizer(relevant_history, max_length=max_history_summary_length, min_length=50)[0]["summary_text"]

    # --- Construct Prompt for Gemini Response ---
    context_str = ""
    if user_id and user_id in user_profiles and user_profiles[user_id]["context"]:  # Check if user profile exists
        context_str = "Here's a summary of the recent conversation:\n"
        for turn in user_profiles[user_id]["context"]:
            # Check if both 'query' and 'response' keys exist
            if 'query' in turn and 'response' in turn:
                context_str += f"User: {turn['query']}\nBot: {turn['response']}\n"
            else:
                logging.warning(f"Incomplete turn in context: {turn}")  # Log a warning for debugging

    prompt = (
        f"You are a friendly and helpful Young Protogen Furry who understands human emotions and social cues and speaks only turkish.  "
        ...
        f"The current dialogue state is: {user_profiles.get(user_id, {}).get('dialogue_state', 'general_conversation')}.  "  # Handle missing user profile
    ...
    if user_id and user_id in user_profiles and user_profiles[user_id]["personality"]:  # Check if user profile exists
    ...
    try:
        if user_id and user_id in user_profiles and user_profiles[user_id]["dialogue_state"] == "planning":  # Check if user profile exists
    ...
    # --- Explainable AI: Add Reasoning Explanation ---
...
async def get_relevant_history(user_id, current_message):
    async with db_lock:
        ...
        # --- Fix: Ensure max_length is not less than min_length ---
        max_summary_length = max(50, len(history_text) // 2)  # Ensure minimum of 50
...
async def on_ready():
    try:
        ...
        try:
            ...
            save_user_profiles()  # Create an empty file if it doesn't exist
        except json.JSONDecodeError as e:
            logging.error(f"Error decoding user profiles file: {e}")
            # Try to fix common JSON errors, invalid characters, and encoding issues
            try:
                user_profiles = fix_json_errors(USER_PROFILES_FILE)
                logging.info("JSON errors, invalid characters, and encoding issues fixed. "
                             "User profiles loaded successfully.")
                save_user_profiles()  # Save the corrected profiles
            except Exception as fix_error:
                logging.error(f"Unable to fix JSON errors: {fix_error}")
                logging.error("Using default user profiles.")
                user_profiles = defaultdict(lambda: {  # Provide default values
                    "preferences": {"communication_style": "friendly", "topics_of_interest": []},
                    "demographics": {"age": None, "location": None},
                    "history_summary": "",
                    "context": deque(maxlen=CONTEXT_WINDOW_SIZE),
...
def fix_json_errors(file_path):
    """
    Attempts to fix common JSON errors, remove invalid characters, and handle
    encoding issues in a file.
    """

    # Try different encodings (UTF-8, UTF-16, Latin-1)
    ...
    # Remove invalid control characters (replace with space)
    ...
    try:
        ...
        # If fixing didn't work, raise the original error
        raise e
...
async def on_message(message):
    try:
        ...
        if bot.user.mentioned_in(message):
            ...
            # --- Store the bot's response in context AND database ---
            ...
            # Handle long responses using Gemini for summarization
            if len(response_text) > 4000:
                await send_long_message(message.channel, response_text)  # Split into multiple messages
            elif len(response_text) > 1000:  # Summarize using Gemini if between 1000 and 4000 chars
                try:
                    ...
                    # Dynamically adjust max_output_tokens
                    ...
                    await send_long_message(message.channel, response_text)  # Fallback to splitting
            else:
                await message.channel.send(response_text)  # Send directly if under 1000 chars

        # --- Save ALL user messages to the database ---
...
async def send_long_message(channel, message_text):
    """Splits a long message into multiple messages within Discord's character limit."""
