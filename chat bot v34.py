...
from google.api_core.exceptions import GoogleAPIError, RetryError # Corrected import
...
üretim_yapılandırması = {"temperature": 0.7, "top_p": 0.95, "top_k": 40, "max_output_tokens": 8192}
gemini_model = GenerativeModel("gemini-1.5-pro-002", generation_config=üretim_yapılandırması)
...
BAĞLAM_PENCERESİ_BOYUTU = 1000000
...
@backoff.on_exception(backoff.expo, (requests.exceptions.RequestException, GoogleAPIError, RetryError), max_time=600)
async def gemini_ile_yanıt_oluştur(istem, kullanıcı_kimliği=None, dil="en"):
    global global_son_istek_zamanı_gemini, global_istek_sayısı_gemini
    ...
    try:
        response = gemini_model.generate_content(istem)
        return response.text.strip()
    except (GoogleAPIError, RetryError) as e:
        logging.error(f"Gemini API Error: {e}")
        return "Gemini ile yanıt oluşturmada hata oluştu. Lütfen tekrar deneyin."
...
async def groq_ile_yanıt_oluştur(istem, kullanıcı_kimliği=None, dil="en"):
    ...
    try:
        completion = client.chat.completions.create(model="llama-3.2-90b-text-preview", messages=[{"role": "user", "content": istem}])
        return completion.choices[0].message.content
    except Exception as e:
        logging.error(f"Groq API Error: {e}")
        return "Groq'tan yanıt almada hata oluştu. Lütfen tekrar deneyin."
...
async def gemini_arama_ve_özetleme(sorgu):
    ...
    try:
        ...
        if arama_sonuçları:
            ...
            istem = f"You are a helpful AI assistant. A user asked about '{sorgu}'. Here are some relevant web search results:\n\n{arama_sonuçları_metni}\n\nPlease provide a concise and informative summary of these search results, prioritizing information relevant to the user's original query.  Cite the URLs used in your summary using [number] notation corresponding to the search results above."
            try:
                yanıt = await gemini_ile_yanıt_oluştur(istem)
    ...
    if kullanıcı_profilleri[kullanıcı_kimliği]["diyalog_durumu"] == "planlama":
        ...
        if kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["aşama"] == "ilk_istek":
            ...
            if await yeterli_planlama_bilgisi_var_mı(kullanıcı_kimliği):
                ...
                plan = await plan_oluştur(kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["hedef"], kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["tercihler"], kullanıcı_kimliği, mesaj)
            ...
            if geri_bildirim_sonucu == "accept":
                ...
                değerlendirme = await planı_değerlendir(kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["plan"], kullanıcı_kimliği)
                ...
                ilk_yürütme_mesajı = await plan_adımını_yürüt(kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["plan"], 0, kullanıcı_kimliği, mesaj)
                return await yanıt_oluştur(kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["plan"], değerlendirme, {}, kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["tercihler"]) + "\n\n" + ilk_yürütme_mesajı
            else:
                kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["aşama"] = "bilgi_toplama"
            ...
            yürütme_sonucu = await plan_yürütmesini_izle(kullanıcı_profilleri[kullanıcı_kimliği]["planlama_durumu"]["plan"], kullanıcı_kimliği, mesaj)
...
async def hedefi_çıkar(sorgu):
    ...
    try:
        hedef = await gemini_ile_yanıt_oluştur(istem)
    except (GoogleAPIError, RetryError) as e:
        logging.error(f"Gemini API Error: {e}")
...
async def çok_gelişmiş_muhakeme_gerçekleştir(içerik, ilgili_geçmiş, özetlenmiş_arama, kullanıcı_kimliği, mesaj, sorgu):
    ...
    try:
        ...
        birleşik_yanıt = f"**Gemini:** {gemini_yanıtı}\n\n**Groq:** {groq_yanıtı}"


        return birleşik_yanıt, "Gemini ve Groq"
...
async def on_message(mesaj):
    ...
    try:
        ...
        result = await çok_gelişmiş_muhakeme_gerçekleştir(içerik, ilgili_geçmiş, özetlenmiş_arama, kullanıcı_kimliği, mesaj, içerik)

        if result:
            yanıt_metni, kaynak = result
            logging.info(f"Yanıt başarıyla {kaynak} tarafından oluşturuldu.")
        else:
