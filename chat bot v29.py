...
async def gemini_search_and_summarize(query) -> str:
    try:
        ddg = AsyncDDGS()
        search_results = await asyncio.to_thread(ddg.text, query, max_results=5)  # Reduce max_results
...
async def perform_very_advanced_reasoning(query, relevant_history, summarized_search, user_id, message, content):
    ...
    user_profiles.setdefault(user_id, {"personality": {"kindness": 0.5, "assertiveness": 0.5}, "dialogue_state": "general_conversation", "context": []})
    ...
    try:
        ...
        """
        try:
            sentiment_response = await generate_response_with_rate_limit(sentiment_prompt, user_id)
        ...
        if user_id:
            if sentiment_label == "positive":
                user_profiles[user_id]["personality"]["kindness"] = min(1, user_profiles[user_id]["personality"].get("kindness", 0) + 0.2)
            elif sentiment_label == "negative":
                user_profiles[user_id]["personality"]["assertiveness"] = min(1, user_profiles[user_id]["personality"].get("assertiveness", 0) + 0.3)
        ...
        try:
            ...
            if not reasoning_analysis:
                logging.warning("Unexpected reasoning response.")
        ...
        next_state = "general_conversation"  # Default fallback state
        ...
        # Ensure the state transition function is callable
        state_transition_function = state_transition_functions.get(next_state)
        if callable(state_transition_function):
            try:
                await state_transition_function(user_id)
            except Exception as e:
                logging.error(f"Error triggering state transition function '{next_state}': {e}")
                # Use a default transition function or fallback action
                fallback_function = state_transition_functions.get("general_conversation")
                if callable(fallback_function):
                    try:
                        await fallback_function(user_id)
                    except Exception as e:
                        logging.error(f"Fallback state transition function 'on_general_conversation' failed: {e}")
        else:
            ...
            if callable(fallback_function):
                try:
                    ...
                    logging.error(f"Fallback state transition function 'on_general_conversation' is not callable or does not exist: {e}")
        ...
        # Step 7: Response Construction Based on Context and History
        ...
        prompt = (
            f"You are a friendly and helpful AI that understands human emotions and uses advanced reasoning. "
        ...
        # Generate final response based on current state
        response_text = "I'm sorry, I encountered an error while processing your request."

        try:
            if user_profiles.get(user_id, {}).get("dialogue_state") == "planning":
        ...
        # Final Fixes and Adjustments to Response (Links, Formatting)
        try:
            response_text = await fix_link_format(response_text)
        except Exception as e:
            logging.error(f"Error fixing link format: {e}")
...
async def on_message(message):
    try:
        ...
        # Check if the user profile exists, create it if it doesn't
        ...
        # Store the user's message in their context
        ...
        # Get relevant history using TF-IDF
        relevant_history = await get_relevant_history(user_id, content)

        # Summarize web search results using Gemini
        ...
        # Store the bot's response in the user's context
