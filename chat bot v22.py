...
# Create the model
generation_config = {
    "temperature": 0.7,  # You can adjust this for creativity vs. factuality
    "top_p": 0.95,
    "top_k": 64,
    ...
    def replay(self, batch_size):
        if len(self.memory) < batch_size:
            ...
            if len(self.memory) > 0:
                td_errors = [abs(
                    experience[2] + self.gamma * np.amax(self.model.predict(experience[3][:self.state_size].reshape(1, -1))[
                        0]) - self.model.predict(experience[0][:self.state_size].reshape(1, -1))[0][experience[1]]) for
...
async def perform_advanced_reasoning(query, relevant_history, summarized_search, user_id):
    ...
    if not is_internet_available():
        return "I'm sorry, I don't have an internet connection right now. Please try again later.", 0
    ...
    if user_id:
        if not user_profiles[user_id]["personality"]:
            recent_sentiments = [sentiment_analyzer.polarity_scores(turn["query"])['compound'] for turn in
    ...
    prompt = (
        f"You are a friendly and helpful Furry Protogen who has a deep understanding of human emotions and social cues and speak always turkish as native level. "
        ...
        # **Enhanced instruction for using search results:**
        f"Use the information from the 'Web Search Summary' below to answer the user's question. If the web search does not provide a direct answer, acknowledge that and try to provide a helpful response based on your general knowledge. \n\nWeb Search Summary:\n{summarized_search}\n\n"
        f"{context_str} "
...
state_size = 2 + len(DIALOGUE_STATES) + 1 + 1 + len(BOT_ACTIONS)  # Updated state size
...
async def on_message(message):
    ...
    try:
        ...
        if bot.user.mentioned_in(message) or bot.user.name in message.content:
            ...
            summarized_search = await duckduckgotool(content)  # Use the function with summarization and error handling
