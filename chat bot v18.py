...
import urllib.parse
import logging
from sklearn.feature_extraction.text import TfidfVectorizer  # For extractive summarization
...
async def safe_send(channel, message_content):
    """Send a message to a Discord channel with rate limit handling."""
    while True:
        try:
            ...
            if e.status == 429:  # Rate limited
                retry_after = e.retry_after  # Seconds to wait before retrying
...
sentence_transformer = AutoModel.from_pretrained('dunzhang/stella_en_1.5B_v5')
...
async def process_db_queue():
    while True:
        (user_id, message, user_name, bot_id, bot_name, selected_action,
         response_time) = await db_queue.get()
        try:
            ...
            async with aiosqlite.connect(DB_FILE) as db:
                await db.execute(
                    ...
                    (user_id, message, datetime.now(timezone.utc).isoformat(),
                     user_name, bot_id, bot_name, selected_action,
                     json.dumps(embedding), response_time))
                await db.commit()
        except Exception as e:
...
async def get_relevant_history(user_id, current_message):
    if sentence_transformer:  # Using semantic similarity
        ...
        inputs = tokenizer(current_message,
                           padding=True,
                           truncation=True,
                           return_tensors="pt")
        with torch.no_grad():
        ...
        for message, similarity in messages[:
                                           5]:  # Take the top 5 most similar messages
...
def process_response(response):
    try:
        # --- Fix link formatting ---
        # Replace square brackets with empty strings to remove Markdown link formatting
        response_text = response.text.replace("[", "").replace("]", "")

        # Validate URLs and include them in the response
        words = response_text.split()
        for word in words:
            if urllib.parse.urlparse(word).scheme in ("http", "https"):
                # No modification needed for valid URLs
                continue
            else:
                # Handle or replace non-URL words if needed
                pass
...
# Asynchronous function to perform DuckDuckGo search
async def duckduckgotool(query) -> str:
    blob = ''
    try:
        ddg = AsyncDDGS()  # Initialize DuckDuckGo search tool
...
async def on_message(message):
    ...
    try:
        ...
        if bot.user.mentioned_in(message) or bot.user.name in message.content:
            ...
            await save_chat_history(user_id, content, user_name, bot_id, response, selected_action, response_time)
