...
if not discord_token or not gemini_api_key or not groq_api_key:
    raise ValueError("DISCORD_BOT_TOKEN or GEMINI_API_KEY or GROQ_API_KEY not set in environment variables")
...
# --- Topic Model ---
class TopicModel:
    def __init__(self, model_type="groq", num_topics=5):  # Changed default to "groq"
        self.model_type = model_type
        self.num_topics = num_topics

        if self.model_type == "groq":
            # Groq-specific initialization (if needed)
            pass
    ...
    async def predict_topic(self, query):  # Changed to async
        if self.model_type == "groq":
            prompt = f"""
        Determine the main topic(s) of the following text:
        "{query}"

        Return a list of the most relevant topics, prioritizing the most relevant.
        """
            topic_string = await groq_ai_function(prompt, model_name="llama-3.1-70b-versatile")
            topic_vector = [topic.strip() for topic in topic_string.split(",")]
            return topic_vector

        elif self.model_type == "lda":
            bow = self.dictionary.doc2bow(query.lower().split())
            topic_distribution = self.lda_model[bow]
            topic_vector = [0] * self.num_topics
    ...
    def update_model(self, documents):
        if self.model_type == "lda":
            bow_corpus = [self.dictionary.doc2bow(doc.lower().split()) for doc in documents]
            self.lda_model.update_alpha(bow_corpus)
...
class ContextStack:
    ...
    def add_context(self, context):  # New method to add context
    ...
    def get_relevant_contexts(self, query):
        ...
        context_tfidfs = self.tfidf_vectorizer.transform([c["query"] for c in self.stack])
        similarities = cosine_similarity(query_tfidf, context_tfidfs)[0]
        sorted_contexts = sorted(zip(self.stack, similarities), key=lambda x: x[1], reverse=True)
        return [c[0] for c in sorted_contexts]

class QuestionAnalyzer:
    def __init__(self):
        self.tfidf_vectorizer = TfidfVectorizer()  # This remains unchanged
...
class GeminiInteraction:
    ...
    def generate_prompt(self, query, topic_history, context_stack, question_type, entities):
        # Construct prompt for Gemini based on context and question analysis
        prompt = f"""
...
"""
    ...
    def learn(self, state, action, reward, next_state):
        ...
        if state not in self.q_table:
            self.q_table[state] = {action: 0}  # Initialize state-action pair if not present
...
async def get_embeddings_groq(text):
    ...
    """
    response = await groq_ai_function(prompt)
    # Yanıtı ayrıştırmak için uygun kodu ekleyin
...
async def get_relevant_history(user_id, current_message):
    ...
    # You can customize the get_relevant_history function further to implement 
    # alternative methods for determining the relevance of previous messages 
    # if you wish. For example, you could use a keyword-based approach or a 
    # more sophisticated text analysis technique.
    return history_text 
...
async def on_message(message):
    ...
    try:
        ...
        if bot.user.mentioned_in(message) or bot.user.name in message.content:
            ...
            # --- Konu Tahmini ---
            topic_model = TopicModel(model_type="groq")  # or "lda"
            topic_vector = await topic_model.predict_topic(content)  # Ensure async call
            user_profiles[user_id]["topic_history"].append(topic_vector) 

            # --- Bağlam Yığınına Ekleme ---
            context = {"query": content, "response": None, "topic": topic_vector}  # Her sohbet turunu kaydet
            user_profiles[user_id]["context_stack"].add_context(context) # Use the new add_context method

            # --- Soru Analizi ---
            question_analyzer = QuestionAnalyzer()
            question_type, entities = await question_analyzer.analyze(content)  # Ensure this is awaited
            ...
            state = (user_profiles[user_id]["dialogue_state"], user_profiles[user_id]["personality"], user_profiles[user_id].get("current_sentiment", 0))
            ...
            # --- Update RL Agent ---
            next_state = (user_profiles[user_id]["dialogue_state"], user_profiles[user_id]["personality"], user_profiles[user_id].get("current_sentiment", 0))
